...
production blockers, and the final Sprint 9 security acceptance gate.
...
### Sprint 3.5 — done in code; hardened workflow redeployment is owner-action
...
2. ✅ **GitHub Actions workflow templates** — staged in
   ...
   publish. The hardened `regenerate-catalog.yml` uses an explicit repository
   allowlist, an immutable Iskra generator revision, SHA-pinned Actions,
   independent release-byte hashing, existing-version immutability, and a
   separate reviewer-gated `signing` environment before publication.
...
Open Sprint 3.5 owner actions:

- Redeploy the hardened templates; the files in this repo are templates and do
  not change the already-active satellite-repo workflows automatically.
- Add tracked `firmware-repos.txt`, create the `signing` environment with a
  required reviewer and no self-review, and move `CATALOG_PRIV_KEY` from the
  repository into that environment.
- Rotate the current dev catalog key before factory use, then approve and test
  the first independently verified signed publication.
...
### Sprint 5 — done in code; station rollout and production log trust remain
...
Current lab trust model: stations share a GitHub App private key scoped to
`iskra-logs`. That separation protects catalog/firmware roots, but a compromised
station can impersonate another station and mutate central history. Sprint 9
therefore requires per-station authenticated, append-only server-side ingestion
before this is considered a production audit trail.
...
5. ✅ **Settings + constants** — `AppSettings.LogShippingEnabled` (default
   ...
   constants are provisioned for the current lab App.
...
- **Distribute the .pem to each station** at
  `%PROGRAMDATA%\Iskra\station-app.pem` with tight ACLs for lab testing only.
- **Deploy the SHA-pinned workflow + script into iskra-logs** and verify an
  empty-repo rebuild plus a real station upload.
- **First end-to-end run:** `Iskra.Cli --ship-logs-now` on a real
  station and confirm a fresh `logs.db` appears in iskra-logs within
  ~30 s.
- **Before factory rollout:** replace the shared repository-wide write key and
  mutable Contents-API log model with the Sprint 9 ingestion architecture.
...
| 6.5 | ⏸️ **Deferred by owner decision (2026-08-08)** — batch mode is off in production, so cross-station locking is not a blocker and building it now would guess at factory network topology. Today `ReserveBatchLock` is per station: two stations on one batch ID keep independent locks. Trigger to build: enabling batches on more than one station sharing a batch ID. Policy already decided — **fail closed, always**; no silent fallback to the local lock. Must bind batch/product/version/digest/target/**station** (the current `BatchLockDescriptor` lacks station identity). Backing store still open; shared network folder leads, GitHub `iskra-logs` rejected for per-flash latency and forgeable locks. See `ROADMAP.md`. |
...
Iskra.sln
src/
  Iskra.Core/        Target-agnostic services, state machine, trust, persistence
  Iskra.Cli/         Console flasher and maintenance/diagnostic tooling
  Iskra.Wpf/         Current Windows operator UI
tests/
  Iskra.Core.Tests/  xUnit Core and security regression suite
installer/           WiX MSI and Windows setup bundle
```
...
The CLI executes the production flash path. Use `--dry-run` for parser and
configuration checks that must not write firmware.
