    ...
    private static readonly IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> ErrorHints =
        new Dictionary<string, IReadOnlyDictionary<string, string>>(StringComparer.Ordinal)
        {
            [IskraLanguages.Ukrainian] = new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                ["E_FW_TOO_LARGE"] = "Прошивка більша за флеш-пам'ять цього мікроконтролера. Обрано невірний виріб.",
                ["E_FW_ADDRESS_RANGE"] = "Адреси прошивки не збігаються з картою пам'яті цього мікроконтролера. Обрано невірний виріб.",
                ["E_FW_READ_FAILED"] = "Не вдалося прочитати файл прошивки. Перевірте права доступу та носій.",
                ["E_AUDIT_WRITE_FAILED"] = "Прошивку записано, але результат не вдалося зберегти в локальному журналі. Відкладіть плату та зверніться до інженера.",
                ["E_AUDIT_PATH_INVALID"] = "Шлях локального журналу небезпечний або непостійний. Виправте його в Налаштуваннях.",
                ["E_SETTINGS_INVALID"] = "Налаштування станції недійсні. Виправте їх перед прошиванням.",
            ...
            [IskraLanguages.English] = new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                ["E_FW_TOO_LARGE"] = "The firmware is larger than this microcontroller's flash. The wrong product is selected.",
                ["E_FW_ADDRESS_RANGE"] = "The firmware addresses do not match this microcontroller's memory map. The wrong product is selected.",
                ["E_FW_READ_FAILED"] = "The firmware file could not be read. Check permissions and the storage device.",
                ["E_AUDIT_WRITE_FAILED"] = "Firmware was written, but the result could not be saved to the local audit log. Quarantine the board and contact an engineer.",
                ["E_AUDIT_PATH_INVALID"] = "The local audit-log path is unsafe or non-persistent. Correct it in Settings.",
                ["E_SETTINGS_INVALID"] = "The station settings are invalid. Correct them before flashing.",
            ...
            [IskraLanguages.German] = new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                ["E_FW_TOO_LARGE"] = "Die Firmware ist größer als der Flash dieses Mikrocontrollers. Es ist das falsche Produkt ausgewählt.",
                ["E_FW_ADDRESS_RANGE"] = "Die Firmware-Adressen passen nicht zur Speicherbelegung dieses Mikrocontrollers. Es ist das falsche Produkt ausgewählt.",
                ["E_FW_READ_FAILED"] = "Die Firmwaredatei konnte nicht gelesen werden. Prüfen Sie Berechtigungen und Datenträger.",
                ["E_AUDIT_WRITE_FAILED"] = "Die Firmware wurde geschrieben, aber das Ergebnis konnte nicht im lokalen Prüfprotokoll gespeichert werden. Legen Sie die Platine beiseite und wenden Sie sich an einen Techniker.",
                ["E_AUDIT_PATH_INVALID"] = "Der Pfad des lokalen Prüfprotokolls ist unsicher oder nicht dauerhaft. Korrigieren Sie ihn in den Einstellungen.",
                ["E_SETTINGS_INVALID"] = "Die Stationseinstellungen sind ungültig. Korrigieren Sie sie vor dem Flashen.",
